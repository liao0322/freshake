...
- (void)initialize {
    ...
    if (self.Status == 0) {
        self.title = @"礼品卡常见问题";
        self.headTitleLabel.text = @"如何激活及其激活注意事项";
        self.useLabel.text = @"如何激活";
        self.remindLabel.text = @"激活注意事项";
        
    } else {
        self.title = @"优惠券常见问题";
        self.headTitleLabel.text = @"如何兑换及其兑换注意事项";
...
- (NSString *)useString {
    if (self.Status == 0) {
        return @"您可以在鲜摇派首页【我的】-【礼品卡】页面输入框里输入“券号”、“密码”激活。";
    }
...
- (NSString *)remindString {
    if (self.Status == 0) {
        return @"1） “激活”是指将鲜摇派礼品卡与鲜摇派账户建立关联的操作。\n2）用户必须先将礼品卡激活到自己的鲜摇派账户之后，才能使用该礼品卡在鲜摇派支付订单。\n3）礼品卡激活到鲜摇派账户后，仅可用于支付本鲜摇派账户的订单，不可支付其他鲜摇派账户的订单，也不可再转移至其他鲜摇派账户。\n温馨提示：通过鲜摇派首页【我的】-【礼品卡】、或参加活动获得赠送的礼品卡，鲜摇派将直接充入您的鲜摇派账户里，无需单独激活哦~ ";
    }
