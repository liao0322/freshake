...
- (void)onResp:(BaseResp *)resp {
    if ([resp isKindOfClass:[SendMessageToWXResp class]]) {
        ...
        switch (resp.errCode) {
            case WXSuccess:
                NSLog(@"支付成功!!!!－0 支付成功 -1 不成功 -2 取消支付，retcode = %d", resp.errCode);
            ...
            default:
                ...
                NSLog(@"支付失败!!!-> 0 支付成功 -1 不成功 -2 取消支付，retcode = %d, retstr = %@", resp.errCode,resp.errStr);
                if (resp.errCode != 0 && resp.errCode != -2)
                {
                    UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"注 意" message:@"支付失败!" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
                    [alter show];
