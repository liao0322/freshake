...
- (void)createUI {
    ...
    .heightIs(self.contentView.size.height);
...
- (void)setModel:(FSCouponModel *)model {
    if (![model.hasLingQu boolValue] && ([model.IsExpile boolValue] || [model.IsExpile intValue] == 2)) {
       ...
       // self.bgImgView = [[UIImageView alloc] initWithImage:bgImage];
        ...
        //self.bgImgView = [[UIImageView alloc] initWithImage:bgImage];
        self.bgImgView.image = bgImage;
        self.priceLabel.textColor = [UIColor colorWithHexString:@"0xe6e6e6"];
...
- (void)setAttributtedStringWithString:(NSString *)string textColor:(UIColor *)color {
    ...
    [attributeString setAttributes:@{NSForegroundColorAttributeName : color, NSFontAttributeName : [UIFont boldSystemFontOfSize:40.0]} range:NSMakeRange(1, string.length)];
